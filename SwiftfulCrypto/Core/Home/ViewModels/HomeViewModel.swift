...
//  SwiftfulCrypto
//
//  Created by Klaudiusz Mękarski on 13/07/2023.
//
...
class HomeViewModel: ObservableObject {
    ...
    private func filterAndSortCoins(text: String, coins: [CoinModel], sortOption: SortOption) -> [CoinModel] {
        ...
        sortCoins(coins: &updatedCoins, sort: sortOption)
        return updatedCoins
    }
    ...
    private func sortPortfolioCoinsIfNeeded(coins: [CoinModel]) -> [CoinModel] {
        // will only sort by holding or holdingReversed
        switch(sortOption) {
        case .holdings:
    ...
    private func mapGlobalMarketData(marketDataModel: MarketDataModel?, portfolioCoins: [CoinModel]) -> [StatisticModel] {
        ...
        let portfolioPercentageChange = ((portfolioValue - previousValue) / previousValue)
